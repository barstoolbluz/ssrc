...
-   **Race Conditions**: While the check and decrement of `nIdleWorkers` are atomic operations, a race condition could theoretically occur between the check and the spawning of a new thread. In the case where `nIdleWorkers` is 1 immediately before a job starts, and due to a race condition it becomes zero after the job starts, another job has started running. There is no situation where all worker threads become awaiting when `nIdleWorkers` is 0; as long as there is at least one executable job, one or more worker threads are executing jobs. Therefore, if the number of jobs is finite, job execution will eventually complete.
